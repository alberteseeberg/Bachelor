...
AlderOP=Q_NO$Alder
NO_døvedata=data.frame(ID,ÅrUdenHørelse,AlderOP, Musikviden)
...
Q_NO$ÅrUdenHørelse=as.integer(Q_NO$ÅrUdenHørelse)
...
#names(Q_ER)
#Renaming------
Q_ER=plyr::rename(Q_ER, c("Dato.for.første.operation"="DatoOp1", "Dato.for.evt..anden.operation"="DatoOp2", "Hvor.gammel.var.du.da.de.høretab.opstod"="AlderHøretab", "Hvor.gammel.var.du.da.dit.høretab.blev.så.alvorligt.at.du.ikke.længere.havde.gavn.af.dit.høreapparat" = "AlderUdenGavnHøreapparat", "Kan.du.tale.i.telefon"="TaleTelefon", "Uddyb.1"="TaleTelefonUD.", "Havde.du.tinnitus.inden.din.CI.operation"="TinnitusPre", "Har.du.tinnitus.nu"="TinnitusPost", "Uddyb.2"="TinnitusUD.", "Bruger.du.høreapparat"="Høreapparat", "Uddyb.3"="HøreapparatUD.",  "Jeg.vælger.at.lytte.til.musik..f.eks..bevidst.at.høre.radio.i.bilen.eller.hjemme..gå.til.koncerter..se.musikprogrammer.i.fjernsynet..osv.."="Lyttevaner"  ))
#--------
...
ER_døvedata=data.frame(ID,ÅrUdenHørelse, AlderOP, Musikviden)
...
Q_ER$ÅrUdenHørelse=as.integer(Q_ER$ÅrUdenHørelse)
...
#709
sum(AllCond$Correct[AllCond$Condition=="Control_e"])
#1095
sum(AllCond$Correct[AllCond$Condition=="Control_y"])
#1506
...
m2=glmer(Correct~Condition*DeviantLevel+(1|ID), data=CIData, family=binomial)
summary(m2)
...
m3=glmer(Correct~Condition*DeviantLevel+Alder+(1|ID), data=CIData, family=binomial)
summary(m3)

anova(m2,m3)


m4=glmer(Correct~Condition*DeviantLevel+Alder+ÅrUdenHørelse+(1|ID), data=CIData, family=binomial)
summary(m4)


M5=glmer(Correct~Condition*DeviantLevel+Alder+ÅrUdenHørelse+Musikviden+(1|ID), data=CIData, family=binomial)
summary(M5)
...
m5=glmer(Correct~Condition*DeviantLevel+(1|ID), data=PitchCI, family=binomial)
summary(m5)


m6=glmer(Correct~Condition*DeviantLevel+Alder+(1|ID), data=PitchCI, family=binomial)
summary(m6)


m7=glmer(Correct~Condition*DeviantLevel+Alder+Musikviden+(1|ID), data=PitchCI, family=binomial)
summary(m7)
...
m8=glmer(Correct~Condition*DeviantLevel+(1|ID), data=RhythmCI, family=binomial, control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(m8)


m9=glmer(Correct~Condition*DeviantLevel+Alder+(1|ID), data=RhythmCI, family=binomial, control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(m9)


m10=glmer(Correct~Condition*DeviantLevel+Alder+Musikviden+(1|ID), data=RhythmCI, family=binomial, control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(m10)
...
m11=glmer(Correct~Condition*DeviantLevel+(1|ID), data=TimbreCI, family=binomial)
summary(m11)


m12=glmer(Correct~Condition*DeviantLevel+Alder+(1|ID), data=TimbreCI, family=binomial)
summary(m12)


m13=glmer(Correct~Condition*DeviantLevel+Alder+Musikviden+(1|ID), data=TimbreCI, family=binomial)
summary(m13)
...
m14=glmer(Correct~Condition*DeviantLevel+(1|ID), data=IntensityCI, family=binomial)
summary(m14)


m15=glmer(Correct~Condition*DeviantLevel+Alder+(1|ID), data=IntensityCI, family=binomial)
summary(m15)


m16=glmer(Correct~Condition*DeviantLevel+Alder+Musikviden+(1|ID), data=IntensityCI, family=binomial, control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(m16)
...
# model1 = glmer(Correct ~ Condition+(1|ID), data=AllCond, family=binomial)
# summary(model1)
# 
# model2= glmer(Correct ~ ÅrUdenHørelse +(1+ÅrUdenHørelse|ID), data=AllData, family = binomial)
# summary(model2)
# 
# model3= glmer(Correct~ÅrUdenHørelse+Alder+(1|ID), data=AllData, family=binomial)
# summary(model3)
# 
# model4=glmer(Correct~ÅrUdenHørelse*Alder+(1|ID), data=AllData, family=binomial)
# summary(model4)
# 
# model5=glmer(Correct~Timer1+Timer2+Musikviden+(1|ID), data=AllData, family=binomial)
# summary(model5)
# 
# model6=glmer(Lyttevaner~Timer1+Timer2+Musikviden+(1|ID), data=AllData, family=binomial)
# 
# 
# 
# model7 = glmer(Correct ~Condition+Deviant+DeviantLevel+(1|ID), data=AllCond, family=binomial)
# summary(model7)
# 
# 
# 
#  model8 = glmer(Correct ~Condition*Deviant*DeviantLevel+(1|ID), data=AllCond, family=binomial, control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
# summary(model8)
# vcov(model8)
